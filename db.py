...
class DB:
    ...
    def remove_if_inroom(self, username):
        port = self.get_peer_ip_port(username)[1]
        room = self.db.rooms.find_one({"peers": {"$in": [port]}})
        if room:
            self.remove_user(room["roomId"], port)
            logging.info(f"User {username} removed from room {room['roomId']}.")
            if len(self.get_room_users(room["roomId"])) == 0:
                self.remove_room(room["roomId"])
                logging.info(f"Room {room['roomId']} removed.")
